...
class MouseMode : public Mode {
 public:
  MouseMode(SSD1306AsciiAvrI2c *Display, KeypadButtons *Buttons,
            uint8_t keyCount = NUM_ALL_BUTTONS, uint8_t grid_width = 6,
            uint8_t grid_height = 8)
      : Mode(Display, Buttons, {}, keyCount, grid_width, grid_height) {}
  ...
  bool mouseMoving(uint8_t physical_button, bool being_released,
                   bool only_reading_value, bool ignore_toggle_check = false);
  bool borderMouse(uint8_t physical_button, bool being_released,
                   bool only_reading_value, bool ignore_toggle_check = false);

  bool mouseButton(uint8_t physical_button, bool being_released,
